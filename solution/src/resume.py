...
def restore_from_checkpoint(config: TrainConfig, checkpoint: dict[str, Any]) -> TrainingState:
    ...
    torch_rng = checkpoint.get("rng_state")
...
def latest_checkpoint(run_store: InMemoryRunStore, checkpoint_store: LocalCheckpointStore | S3CheckpointStore, config: TrainConfig) -> tuple[str, dict[str, Any]]:
    ...
    for candidate in checkpoints:
        try:
            checkpoint = load_checkpoint(checkpoint_store, candidate["checkpoint_key"])
            issues = validate_checkpoint_metadata(checkpoint, config)
            if issues:
                errors.append(f"{candidate['checkpoint_key']}: {issues}")
                continue
            return candidate["checkpoint_key"], checkpoint
        except Exception as exc:  # noqa: BLE001
            errors.append(f"{candidate['checkpoint_key']}: {exc}")
            continue
    raise RuntimeError(f"no valid checkpoints found: {errors}")
