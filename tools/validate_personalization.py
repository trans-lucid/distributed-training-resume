...
def profile_for(difficulty: str, focus: str, seed: int, entity_count: str) -> dict:
    return {
        ...
        "role": "ML Infrastructure Engineer" if difficulty != "staff" else "Staff ML Infrastructure Engineer",
        "time_limit": "60 min" if difficulty == "junior" else "120 min" if difficulty == "staff" else "90 min",
        "theme": f"{difficulty.title()} scenario for AcmeOps deterministic checkpoint recovery",
        ...
        "personalization": {
            "business_nouns": ["training run", "checkpoint", "fine-tuning job"],
            "scenario_names": [f"{difficulty}-factory-scenario"],
            "fixture_field_names": ["run_id", "checkpoint_key"],
            "hidden_test_emphasis": ["deterministic resume", "hardcoding resistance"],
        },
    }
    ...
    try:
        ...
        for profile in profiles:
            ...
            if manifest.get("template_slug") == "workflow-recovery-reconciler":
                ...
                snapshots[profile["difficulty"] + "_fixture"] = fixture.read_bytes()
            if manifest.get("template_slug") == "tenant-permission-audit":
                fixture = MAIN / "fixtures" / "public" / "generated_access_cases.jsonl"
                if not fixture.exists():
                    raise AssertionError("tenant personalized render missing generated access fixture")
                rows = [json.loads(line) for line in fixture.read_text().splitlines() if line.strip()]
                if not rows:
                    raise AssertionError("tenant personalized fixture is empty")
                if not any(row.get("failure_mode") != "normal" for row in rows):
                    raise AssertionError("tenant personalized fixture does not include permission drift cases")
                if not any(row.get("expected_policy_evidence") for row in rows):
                    raise AssertionError("tenant personalized fixture does not require policy evidence")
                snapshots[profile["difficulty"] + "_fixture"] = fixture.read_bytes()
            if manifest.get("template_slug") == "distributed-training-resume":
                fixture = MAIN / "fixtures" / "public" / "generated_training_profile.json"
                if not fixture.exists():
                    raise AssertionError("distributed personalized render missing generated training profile")
                payload = json.loads(fixture.read_text())
                cases = payload.get("checkpoint_cases", [])
                if not cases:
                    raise AssertionError("distributed personalized fixture missing checkpoint cases")
                if not any(case.get("requires_full_state") for case in cases):
                    raise AssertionError("distributed personalized fixture does not require full training state")
                if "dataset_summary" not in payload or payload["dataset_summary"].get("row_count", 0) <= 0:
                    raise AssertionError("distributed personalized fixture missing dataset variation evidence")
                snapshots[profile["difficulty"] + "_fixture"] = fixture.read_bytes()
            if manifest.get("template_slug") == "async-webhook-ledger":
        ...
        if manifest.get("template_slug") == "tenant-permission-audit" and snapshots["junior_fixture"] == snapshots["staff_fixture"]:
            raise AssertionError("tenant fixture output did not change between junior and staff")
        if manifest.get("template_slug") == "distributed-training-resume" and snapshots["junior_fixture"] == snapshots["staff_fixture"]:
            raise AssertionError("distributed fixture output did not change between junior and staff")
