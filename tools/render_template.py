...
TEMPLATE_TITLE = "Distributed Training Resume"
PROFILE_ARTIFACT = Path("fixtures/public/generated_training_profile.json")
...
def focus(context: dict) -> str:
    ...
    return _safe_scalar(selected.get("focus") or context.get("focus") or "deterministic resume", 160)
...
def scenario_summary(context: dict) -> str:
    ...
    if nouns:
        return f"Recover deterministic training progress for {', '.join(nouns)} without losing state."
    return f"Repair the {TEMPLATE_TITLE} production path for a realistic ML infra preemption scenario."
...
def write_solution_personalization(solution_root: Path) -> None:
    ...
    lines = [
        ...
        "",
        "## Public Test Purpose",
        "Public tests verify checkpoint completeness, deterministic split-resume, corrupt checkpoint fallback, and Docker-backed MinIO/Postgres writes.",
        "",
        "## Hidden Test Intent",
        "Hidden tests exercise config mismatch rejection, corrupt latest fallback, RNG sample order, optimizer loss continuity, repeated resume idempotency, and report issue classification.",
        "",
        "## Scoring Rubric",
        "Score against the selected focus, complete checkpoint state, optimizer/scheduler/RNG continuity, metadata validation, corrupt checkpoint handling, and recovery report quality.",
        "",
        "## Debrief Answer Cues",
        "Strong answers should explain full training-state checkpointing, deterministic baseline comparison, config-hash validation, corrupt checkpoint fallback, and recoverable versus unrecoverable report classification.",
        "",
